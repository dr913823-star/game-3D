...
(function (global) {
    ...
    global.NPC_MESH_CREATORS['villager'] = function createAldeao(def) {
        ...
        // Ombros
        const shGeo = new THREE.SphereGeometry(0.17 * s, 10, 8);
        const lSh = new THREE.Mesh(shGeo, matCloth);
        lSh.position.set(-0.44 * s, 0.2 * s, 0);
        lSh.castShadow = true;
        torso.add(lSh);
        const rSh = new THREE.Mesh(shGeo, matCloth);
        rSh.position.set(0.44 * s, 0.2 * s, 0);
        rSh.castShadow = true;
        torso.add(rSh);
        ...
        const handGeo = new THREE.SphereGeometry(0.07 * s, 8, 6);
        ...
        const bootGeo = new THREE.BoxGeometry(0.15 * s, 0.12 * s, 0.26 * s);
        const matBoot = new THREE.MeshStandardMaterial({ color: 0x292524, roughness: 0.9 });
        const lBoot = new THREE.Mesh(bootGeo, matBoot);
        lBoot.position.set(-0.14 * s, 0.07 * s, 0.04 * s);
        group.add(lBoot);
        const rBoot = new THREE.Mesh(bootGeo, matBoot);
        rBoot.position.set(0.14 * s, 0.07 * s, 0.04 * s);
        group.add(rBoot);
