...
(function (global) {
    ...
    global.NPC_MESH_CREATORS['healer'] = function createCurandeira(def) {
        ...
        lSh.position.set(-0.42 * s, 0.2 * s, 0);
        lSh.castShadow = true;
        torso.add(lSh);
        const rSh = new THREE.Mesh(shGeo, matAccent);
        rSh.position.set(0.42 * s, 0.2 * s, 0);
        rSh.castShadow = true;
        torso.add(rSh);
        ...
        const vial = new THREE.Mesh(
            new THREE.CylinderGeometry(0.05 * s, 0.06 * s, 0.2 * s, 8),
            new THREE.MeshStandardMaterial({
                color: 0x22c55e, emissive: 0x16a34a, emissiveIntensity: 0.4,
                transparent: true, opacity: 0.85
            })
        );
        vial.position.set(0.5 * s, armY - 0.2 * s, 0.15 * s);
        group.add(vial);
        const cork = new THREE.Mesh(new THREE.CylinderGeometry(0.04 * s, 0.04 * s, 0.05 * s, 6), matAccent);
        cork.position.set(0.5 * s, armY - 0.08 * s, 0.15 * s);
        group.add(cork);
        ...
        // Nome ACIMA do capuz (capuz chega ~ headY + 0.14 + 0.36 ≈ 2.48*s)
